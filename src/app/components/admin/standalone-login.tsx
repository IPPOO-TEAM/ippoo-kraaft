/**
 * Page de login standalone - chargée SANS tous les providers de l'app
 * pour un chargement ultra-rapide
 */
import { useEffect, useState } from "react";
import { useNavigate, Navigate, useLocation } from "react-router";
import { Loader2, Lock, ShieldCheck, AlertTriangle } from "lucide-react";
import { MinimalAdminProvider, useMinimalAdmin } from "./minimal-admin-provider";
...
function LoginForm() {
  const { session, login, lockedUntil } = useMinimalAdmin();
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--ipk-ink)] p-4">
      <form onSubmit={onSubmit} className="w-full max-w-md bg-white rounded-2xl p-6 sm:p-8 shadow-xl">
        ...
        <div className="space-y-4">
          ...
          <button type="submit" disabled={loading || isLocked} className="w-full bg-[var(--ipk-green-dark)] hover:bg-[var(--ipk-green-darker)] text-white rounded-xl h-11 disabled:opacity-60 flex items-center justify-center gap-2" style={{ fontSize: "14px", fontWeight: 600 }}>
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Connexion…</> : <><Lock className="w-4 h-4" /> Se connecter</>}
          </button>
        </div>
...
export function StandaloneAdminLoginPage() {
  return (
